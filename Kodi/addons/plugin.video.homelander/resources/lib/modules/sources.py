...
class sources:
    ...
    def getMovieSource(self, title, localtitle, aliases, year, imdb, source, call):
        ...
        try:
            ...
            try:
                sources = call().sources(data, self.sourceDict)
            except:
                sources = call.sources(data, self.sourceDict, self.hostprDict)
    ...
    def getEpisodeSource(self, title, year, imdb, tmdb, season, episode, tvshowtitle, localtvshowtitle, aliases, premiered, source, call):
        ...
        try:
            ...
            sources = call().sources(data, self.hostDict)
            if sources == None or sources == []: raise Exception()
            sources = [json.loads(t) for t in set(json.dumps(d, sort_keys=True) for d in sources)]
            for i in sources: i.update({'provider': source})
            self.sources.extend(sources)
    ...
    def sourcesProcessTorrents(self, torrent_sources):#adjusted Fen code
        try:
            ...
            DBCheck = debridcheck.DebridCheck(imdb=self.imdb, season=self.season, episode=self.episode)
            ...
            for i in uncachedTorrents: i.update({'source': 'unchecked torrent'})
    ...
    def getConstants(self):
        ...
        if externalEnabled == 'true':
            try:
                ...
                self.module_name = 'homelanderscrapers'
                """
                import homelanderscrapers
                self.sourceDict += homelanderscrapers.sources()
                self.module_name = 'homelanderScrapers (' + str(control.addon('script.module.homelanderscrapers').getSetting('package.folder')) + ' set):' \
                                   if control.addon('script.module.homelanderscrapers').getSetting('package.folder') != 'homelanderscrapers' else '[COLOR red]homelander Is Searching[/COLOR]'
                """
            except:
                pass
