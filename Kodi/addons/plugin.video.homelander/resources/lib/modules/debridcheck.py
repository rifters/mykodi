...
##-- homelander add-on's debrid cache check module adjusted for homelander/Exodus code base - all credits to Tikipeter --##
...
class RDapi:
    ...
    def check_cache(self, hashes, imdb:'', season='', episode=''):
        """
        ...
        url = (
            f'https://torrentio.strem.fun/realdebrid=xxxxxxxxxxxxxxxxxxxxxxxxxxxx/stream/movie/{imdb}%3A{season}%3A{episode}.json'
            if season and episode else
            f'https://torrentio.strem.fun/realdebrid=xxxxxxxxxxxxxxxxxxxxxxxxxxxx/stream/movie/{imdb}.json'
        )
...
class ADapi:
    ...
    def check_cache(self, hashes, imdb='', season='', episode=''):
        ...
        headers = {"User-Agent": user_agent}
        url = (
            f'https://torrentio.strem.fun/alldebrid=xxxxxxxxxxxxxxxxxxxxxxxxxxxx/stream/movie/{imdb}%3A{season}%3A{episode}.json'
            if season and episode else
            f'https://torrentio.strem.fun/alldebrid=xxxxxxxxxxxxxxxxxxxxxxxxxxxx/stream/movie/{imdb}.json'
        )
        response = requests.get(url, headers=headers, timeout=20).text
        try: 
            resp = utils.json_loads_as_str(response)
...
class DebridCheck:
    def __init__(self, imdb='', season='', episode=''):
        self.imdb = imdb
        self.season = season
    ...
    def _rd_lookup(self, chunk):
        try:
            rd_cache_get = RDapi().check_cache(chunk, imdb=self.imdb, season=self.season, episode=self.episode)
            streams = rd_cache_get.get('streams', [])
            cached_string = ''
            for stream in streams:
                if '[RD+]' in stream.get('name'):
                    cached_string += stream['url']
            
            for h in chunk:
                cached = 'False'
                #if _hash in response.lower() and _hash not in cached_hashes:
                if h.lower() in cached_string.lower():
    ...
    def _ad_lookup(self, hash_list):
        try:
            ad_cache = ADapi().check_cache(hash_list, imdb=self.imdb, season=self.season, episode=self.episode)
            streams = ad_cache.get('streams', [])
            cached_string = ''
            for stream in streams:
                cached_string += stream['url']
            ...
            for h in hash_list:
                cached = 'False'
                if h.lower() in cached_string.lower():
                    self.ad_cached_hashes.append(h)
                    cached = 'True'
                self.ad_process_results.append((h, cached))
            """
