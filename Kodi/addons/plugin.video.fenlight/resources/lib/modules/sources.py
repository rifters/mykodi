...
class Sources():
	...
	def get_meta(self):
		# Always start with a dict so .update() never crashes
		self.meta = {}

		if self.media_type == 'movie':
			meta = metadata.movie_meta('tmdb_id', self.tmdb_id, settings.tmdb_api_key(), settings.mpaa_region(), get_datetime())
			if meta:
				self.meta = meta
		else:
			try:
				...
				if meta:
					...
					self.meta.update({
						...
						'tvshow_plot': self.meta.get('plot'),
