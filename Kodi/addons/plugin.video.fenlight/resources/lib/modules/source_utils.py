...
def make_alias_dict(meta, title):
    ...
    if alternative_titles:
        aliases.extend([{'title': i, 'country': ''} for i in alternative_titles])

    # Add original title if not already included
    if original_title not in alternative_titles:
        aliases.append({'title': original_title, 'country': ''})

    # Add country‑based aliases
    if country_codes:
        codes = set([i.replace('GB', 'UK') for i in country_codes])
        aliases.extend([{'title': f'{title} {i}', 'country': ''} for i in codes])
