...
#			self.delete_torrent(torrent_id) # cannot delete the torrent, play link will not persist, will return 502
	...
	def user_cloud(self, request_id=None, check_cache=True):
		string = 'coalition_oc_user_cloud_info_%s' % request_id if request_id else 'coalition_oc_user_cloud'
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('coalition_oc_user_cloud%',))
				try:
