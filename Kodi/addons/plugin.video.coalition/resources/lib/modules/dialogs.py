...
def mdb_manager_choice(params):
	...
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/mdblist.png')
...
def favorites_choice(params):
	...
	if params.get('cache'):
		...
		if any(i['tmdb_id'] == tmdb_id for i in current_favourites): action, text = favourites_cache.remove_from_favourites, '%s coalition %s?' % (ls(32603), ls(32453))
		else: action, text = favourites_cache.add_to_favourites, '%s coalition %s?' % (ls(32602), ls(32453))
...
#	icon = translate_path('special://home/addons/script.module.fenomscrapers/icon.png')
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/lib/fenom/media/icon.png')
