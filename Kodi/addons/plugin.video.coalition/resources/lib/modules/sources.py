...
class Sources():
	...
	def _monitor(self):
		kodi_utils.set_property('coalition.progress_is_alive', 'true')
		while kodi_utils.get_property('coalition.progress_is_alive') == 'true': sleep(200)
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, highlight=None, download=False):
		...
		}.get(debrid_provider) or 'coalition.png'
	...
	def play_file(self, results, source={}, autoplay=False, background=False):
		def _uncached_confirm(item):
			if not confirm_dialog(text=ls(32831) % item['debrid'].upper()):
				kodi_utils.clear_property('coalition.progress_is_alive')
				return None
...
#			if not background: self._make_progress_dialog()
			if not background:
				if not self.load_action:
					...
					progressBG.create('coalition', 'coalition loading...')
