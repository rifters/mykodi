...
fanart_default = 'special://home/addons/plugin.video.coalition/fanart.png'
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc is None: cacheToDisc = get_property('coalition_kodi_menu_cache') == 'true'
...
def ok_dialog(heading='coalition', text='', highlight='dodgerblue', ok_label=local_string(32839), top_space=False):
...
def confirm_dialog(heading='coalition', text='', highlight='dodgerblue', ok_label=local_string(32839), cancel_label=local_string(32840), top_space=False, default_control=11):
#	from windows import open_window
	if isinstance(heading, int): heading = local_string(heading)
	if isinstance(text, int): text = local_string(text)
...
def clear_view(view_type):
	...
	try:
		...
		for item in dbcur.fetchall():
			...
			clear_property('coalition_%s' % item[0])
		kodi_db = translate_path('special://home/userdata/Database/ViewModes6.db')
		dbcon = database.connect(kodi_db)
		dbcur = dbcon.cursor()
		dbcur.execute("""DELETE FROM view WHERE path LIKE 'plugin://plugin.video.coalition/%'""")
...
def build_url(url_params):
	return f"plugin://plugin.video.coalition/?{urlencode(url_params)}"
...
def clean_settings():
	...
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
#	progressDialog.create('coalition', '')
