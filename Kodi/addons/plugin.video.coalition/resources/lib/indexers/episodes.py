...
poster = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/media/box_office.png')
	...
	def run(self):
		try:
			...
			if   'in_progress' in mode:
				...
				else: self.list_type = 'next_episode_coalition'
	...
	def build_episode_content(self, _position, ep_data):
		try:
			...
			if self.list_type.startswith('next_episode'):
				...
				props = {'coalition_last_played': last_played}
			else: props = {'coalition_sort_order': string(ep_data_get('sort', _position))}
			cm = []
			cm_append = cm.append
			...
			if not episode_date or self.current_date < episode_date:
				...
				unaired = True
				props['coalition_unaired'] = 'true'
			else:
				unaired = False
				props['coalition_unaired'] = 'false'
			...
			if not unaired:
				if progress != '0' or resumetime != '0':
					...
					props['coalition_in_progress'] = 'true'
				if playcount:
			...
			props['coalition_name'] = '%s - %sx%s' % (title, str_season_zfill2, str_episode_zfill2)
			props['coalition_first_aired'] = premiered
			if self.is_widget: props.update({
				'coalition_widget': 'true',
				'coalition_playcount': string(playcount),
				'coalition_browse_params': browse_params,
				'coalition_browse_seas_params': browse_seas_params,
				'coalition_options_menu_params': options_params,
				'coalition_extras_menu_params': extras_params,
				'coalition_unwatched_params': unwatched_params,
				'coalition_watched_params': watched_params,
				'coalition_clearprog_params': clearprog_params})
			else: props['coalition_widget'] = 'false'
...
#		threads = list(make_thread_list_enumerate(self.build_episode_content, self.list, Thread))
		...
		if self.list_type.startswith('trakt_dict'):
			...
			def func(function):
				if sort_key == 'coalition_name': return title_key_function(function, ignore_articles())
			...
			if nextep_settings['sort_airing_today_to_top']:
				...
				airing_today.sort(key=lambda i: i[1].getProperty('coalition_first_aired'))
				...
				unaired = [i for i in self.items if i[1].getProperty('coalition_unaired') == 'true']
			...
			if self.list_type in ('trakt_calendar', 'trakt_recently_aired'):
				...
				self.items.sort(key=lambda k: k[1].getProperty('coalition_first_aired'), reverse=reverse)
			else: self.items.sort(key=lambda k: int(k[1].getProperty('coalition_sort_order')))
		return self.items
