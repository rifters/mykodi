...
class Navigator:
	...
	def my_content(self):
		...
		t_str, user_str, l_str, ai_str, ml_str, cln_str = ls(32037), ls(32065), ls(32501), ls(32494), ls(32454), 'Clean Watchlist'
		...
		if mdblist_status:
			...
			self._add_item({'mode': 'mdblist.clean_watchlist'                               , 'name': cln_str}, 'mdblist.png', m_n_ins, False)
		if imdb_status:
	...
	def settings(self):
		...
		self._add_item({'mode': 'open_settings',                 'name': coalition_str                 }, 'coalition.png', n_ins, False)
	...
	def log_utils(self):
		coalition_vstr, log_path = k.addon().getAddonInfo('version'), 'special://home/addons/%s/changelog.txt'
		kl_loc, mt_str = tp('special://logpath/kodi.log'), tp(log_path % 'plugin.video.coalition')
		coalition_str, cl_str, lut_str, k_str, lv_str = ls(32036), ls(32508), ls(32777), ls(32538), ls(32509)
		mh_str, klv_h, klu_h = '%s  [I](v.%s)[/I]' % (coalition_str, coalition_vstr), '%s %s' % (k_str, lv_str), ls(32853)
